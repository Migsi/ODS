...
by ``service_id``, and keyed by exact ``record_sha256``.  Public operations
support strict read, create with exact replay, compare-and-replace by prior
``record_sha256``, and compare-and-delete by exact ``record_sha256``.  The
primary ``publish`` path accepts the exact plan-bound ``LifecycleWorkCommand``
...
fabricated dict.  Divergence, stale compare values, malformed state, partial
state, and ambiguous I/O all fail closed with stable value-free codes.  Results
are frozen typed dataclasses with explicit duplicate/replay distinction; no
paths, configuration, or supplied bad values leak through.
...
success without exact post-state proof.  Mutating inputs are cloned/
