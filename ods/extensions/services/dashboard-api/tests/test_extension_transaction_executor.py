...
def test_lockfile_custody_read_failure_maps_to_finalizer_integrity_error(
    ...
    def fail_read():
        raise lockfile_mod.ExtensionLockfileError("lockfile-symlink")

    monkeypatch.setattr(lockfiles, "read", fail_read)
    with pytest.raises(transactions.IntegrityError) as caught:
        make_executor(store, adapter=adapter, finalizer=finalizer).execute(
            descriptor["transactionId"], envelope["planHash"]
        )

    assert caught.value.code == "lockfile-finalization-read-failed"
    assert caught.value.detail == "lockfile-symlink"
    assert store.read(descriptor["transactionId"])["state"] == "committed"
