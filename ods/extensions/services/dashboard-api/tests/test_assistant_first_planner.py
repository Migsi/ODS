...
def test_bundled_searxng_v2_is_the_deterministic_web_search_provider() -> None:
    service_dir = Path(__file__).resolve().parents[2] / "searxng"
    record = yaml.safe_load((service_dir / "manifest.yaml").read_text(encoding="utf-8"))
    record["_catalog"] = {
        "definition_sha256": "sha256:" + "d" * 64,
