...
class FakeDocker:
    ...
    def __call__(self, argv: list[str]) -> subprocess.CompletedProcess[bytes]:
        ...
        if argv[1:3] == ["ps", "-a"]:
            ...
            output = b"" if not self.containers else (IDS[0] + "\n" + IDS[1] + "\n").encode()
        elif argv[1] == "inspect":
            container_id = argv[-1]
...
def _installed(tmp_path: Path):
    ...
    command = fixtures._command(
        f"apply:{fixtures.SERVICE_ID}", [fixtures.SERVICE_ID],
    ...
    model = fixtures._build_canonical_record(
        identity, config_sha=digest_mod.canonical_document_sha256(CONFIG),
...
def test_exact_current_app_is_applied_without_claiming_health(tmp_path):
    ...
    adapter = adapter_mod.ApplicationObservationAdapter(
        tmp_path, FakeRecords(record), FakeReceipts(receipt),
        lambda _command: bound, lambda: True, docker,
    )
    ...
    assert len([call for call in docker.calls if call[1] == "inspect"]) == 4
...
def test_no_files_record_or_containers_is_absent(tmp_path):
    ...
    for name in ("manifest.yaml", "compose.yaml", "configuration.json"):
        (service / name).unlink()
    adapter = adapter_mod.ApplicationObservationAdapter(
        tmp_path, FakeRecords(None), FakeReceipts(fixtures._absent_snapshot()),
        lambda _command: bound, lambda: True, FakeDocker(identity, containers=False),
    )
...
def test_partial_or_unavailable_evidence_never_reports_success(tmp_path, failure, code):
    ...
    adapter = adapter_mod.ApplicationObservationAdapter(
        tmp_path, FakeRecords(None if failure == "missing-record" else record),
        FakeReceipts(receipt), lambda _command: bound, lease,
        FakeDocker(identity, unavailable=failure == "docker-down", drift=failure == "docker-drift"),
    )
    with pytest.raises(adapter_mod.ApplicationEvidenceError if code.startswith("application-evidence") else Exception) as error:
        adapter(command)
...
def test_symlinked_active_file_fails_closed(tmp_path):
    ...
    adapter = adapter_mod.ApplicationObservationAdapter(
        tmp_path, FakeRecords(record), FakeReceipts(receipt),
        lambda _command: bound, lambda: True, FakeDocker(identity),
    )
